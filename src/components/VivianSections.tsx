...
import ZzzRules from "./ZzzRules";

// ─────────────────────────────────────────────────────────────
// Public API to control the ZZZ (Vivian) section from Landing
// ─────────────────────────────────────────────────────────────
export type VivianSectionHandle = {
  openDraftModal: () => void;
  openMatchesModal: () => void;
  // convenience for Landing buttons that want to trigger draft start validation
  startDraftNow: () => void;
  openRulesModal: () => void; 
};

type Props = {
  /** Whether Landing currently has the Vivian tab selected */
  active: boolean;
...
// ─────────────────────────────────────────────────────────────
// Internal helpers/types (moved from Landing)
// ─────────────────────────────────────────────────────────────

const ENABLE_ZZZ_SESSIONS_CHECK = true;
...
// ─────────────────────────────────────────────────────────────
// VivianSection (ZZZ-only modal logic + data)
// Renders only modals/hidden inputs. Use methods via ref.
// ─────────────────────────────────────────────────────────────
const VivianSection = forwardRef<VivianSectionHandle, Props>(
  function VivianSection({ active, isMobile }, ref) {
    ...
    const resumeUnfinished = () => {
      ...
      // prime session keys so /zzz/draft loads the session
    ...
    // ── NEW (moved from Landing): fuzzy key normalizer + lookups builder
    const normalizeKey = (s: string) =>
      s
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "") // strip diacritics
        .replace(/[\s\u00A0\u2000-\u200B\u202F]/g, "") // strip spaces (incl NBSP/thin)
        .replace(/[^a-z0-9]/g, ""); // strip punctuation

    const buildLookups = (
      cMeta: Record<string, any>,
      wMeta: Record<string, any>
    ) => {
      // Characters
    ...
    const handleImportCombinedCsv = async (
      ...
      try {
        // Fresh meta snapshot (don’t rely on possibly-stale state)
        ...
        // Build lookups against the fresh snapshot
        const {
          charNameToCode,
          charNameKeyToCode,
          charCodeToCode,
        ...
        // Header index maps (kept nullable while we parse)
        let cIdx: CharHeaderIdx | null = null;
        let wIdx: WengHeaderIdx | null = null;

        // Remember the most recent header row seen for each section
        let lastCharHeader: CharHeaderIdx | null = null;
        let lastWengHeader: WengHeaderIdx | null = null;
          ...
          if (mode === "weng" && wIdx) {
            ...
            if (nm) {
              id =
                ...
                wengPairKeyToId.get(
                  `${normalizeKey(nm)}|${normalizeKey(sb)}`
                ) ||
                "";
    ...
    // Rules modal (Vivian-specific)
    const [showRulesModal, setShowRulesModal] = useState(false);

    // Matches (live/recent) modal + pagination
    ...
    const goSpectator = (key: string) => {
      // small delay for “leaving” effect (handled in Landing visuals)
    ...
    useEffect(() => {
      if (!ENABLE_ZZZ_SESSIONS_CHECK || !active || !user) {
        setOwnerOpen(null);
        return;
      }
      let cancelled = false;
    ...
    useImperativeHandle(ref, () => ({
      openDraftModal: () => {
        if (ownerOpen) {
          toast.warn("You have an unfinished Vivian PvP draft.");
          setShowResumeModal(true);
          return;
        }
        setShowDraftModal(true);
    ...
    return (
      <>
                  ...
                  {Array(is3v3 ? 3 : 2)
                    ...
                    .map((_, i) => (
                      <input
                        ...
                        maxLength={40}
                        onChange={(e) => {
                          const next = ensureLen(team1Names, is3v3 ? 3 : 2);
                  ...
                  {Array(is3v3 ? 3 : 2)
                    ...
                    .map((_, i) => (
                      <input
                        ...
                        maxLength={40}
                        onChange={(e) => {
                          const next = ensureLen(team2Names, is3v3 ? 3 : 2);
                          next[i] = e.target.value ?? "";
                          setTeam2Names(next);
                        }}
                        style={{
        ...
        >
          ...
          <Modal.Body>
            <p className="mb-2">You have an unfinished Vivian PvP draft:</p>
            <div
